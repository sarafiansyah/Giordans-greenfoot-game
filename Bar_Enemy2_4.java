 ...
 */
public class Bar_Enemy2_4 extends Actor
{
...
//  The color of the bar AT the breakpoint will be the average color between the safe color and the danger color
    ...
    /**
     * enemyBar Constructor: saves the initial values that are brought in and creates the bar image through the 'add(int)' call,
     ...
     */
    public Bar_Enemy2_4(String refText, String unitType, int initValue, int maxValue)
    {
...
private void moveEnemy()
    {
        ...
        int hpmovspeed = 5;
        // Enemy1_1
        java.util.List Enemy2_4 = getWorld().getObjects(Enemy2_4.class);
        if (! Enemy2_4.isEmpty()){
            Enemy2_4 actor = (Enemy2_4)Enemy2_4.get(0);
            turnTowards(actor.getX(), actor.getY()-45);
            
            move(hpmovspeed);
            setRotation(0); // to maintain upright actor
        }
        if(isTouching(Enemy2_4.class)){
